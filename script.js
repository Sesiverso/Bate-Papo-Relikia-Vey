...
// Função para selecionar a imagem de perfil
...
document.getElementById('profile-setup-button').addEventListener('click', () => {
    const username = document.getElementById('username').value.trim();
    const profileImage = localStorage.getItem('profileImage');

    if (username === '' || !profileImage) {
        alert('Por favor, insira um nome e selecione uma imagem de perfil.');
        return;
    }
...
// Mostrar a tela de configuração de perfil ao carregar a página se não houver nome de usuário
window.onload = () => {
    const username = localStorage.getItem('username');
    const profileImage = localStorage.getItem('profileImage');
